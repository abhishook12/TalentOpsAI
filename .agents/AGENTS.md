...
7. **Strict 400 MB Database Size Limit:** Whatever work or data ingestion is done at any time and to any extent, **the total database size MUST NOT exceed the hard limit of 400 MB**. Always monitor, optimize indexes/types, deduplicate strings, and structure storage strictly to guarantee the database stays below 400 MB.
8. **Platform 70% Limitation Safety Shield & Early Alarm System:** Any platform, hosting provider, or API utilized by this project (e.g., Supabase 5 GB monthly egress/500 MB storage limit, Gemini 15 RPM / 1,000 RPD rate limits, Render 512 MB RAM / 0.1 CPU limit, Vercel 100 GB bandwidth limit) MUST NOT exceed **70% of its maximum platform quota or rate limit** during any operation or task. Whenever any resource consumption hits or crosses the **70% mark** (`3.5 GB` Supabase bandwidth, `350 MB` DB size, `10 RPM` API rate, or `358 MB` server memory), the system MUST immediately halt or throttle non-essential background tasks and trigger an **ALARM** to notify the user instantly so limits are never breached.
