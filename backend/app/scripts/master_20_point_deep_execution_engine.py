...
def execute_master_engine():
    print("=== STARTING MASTER 20-POINT DEEP EXECUTION ENGINE ===")
    t0 = time.time()
    conn = psycopg.connect(DB_URL)
    cur = conn.cursor()

    # Build lookup map of canonical companies
    cur.execute("SELECT company_id, company_name FROM companies")
    comp_map = {}
    for cid, cname in cur.fetchall():
        comp_map[cname.lower().strip()] = cid

    # POINT 1 & 11: Domain Concordance & Subsidiary Parent Mapping
    print("\n[Point #1 & #11] Executing Domain Concordance & Subsidiary Parent Mapping...")
        ...
        if dom in DOMAIN_MAP:
            ...
            if target_name in comp_map:
                ...
                if curr_cid != target_cid:
                    ...
                    stats[target_name] = stats.get(target_name, 0) + 1

    print(f" -> Found {len(domain_updates):,} domain misalignments across the database!")
    for name, count in sorted(stats.items(), key=lambda x: x[1], reverse=True)[:10]:
        print(f"    * Realigning {count:,} recruiters to {name.title()}")

    # Execute individual updates in transaction mode to avoid Supabase prepared statement error
    for target_cid, rid in domain_updates:
        cur.execute("UPDATE recruiters SET company_id = %s WHERE recruiter_id = %s", (target_cid, rid))
    conn.commit()
    print(" -> Point #1 & #11 committed successfully!")

    # POINT 6: NANP Phone Area Code vs State Concordance
    print("\n[Point #6] Executing NANP Area Code vs State Concordance...")
    cur.execute("SELECT recruiter_id, phone, phone2, state FROM recruiters WHERE state IS NULL OR state = 'US' OR state NOT IN ('AL','AK','AZ','AR','CA','CO','CT','DE','FL','GA','HI','ID','IL','IN','IA','KS','KY','LA','ME','MD','MA','MI','MN','MS','MO','MT','NE','NV','NH','NJ','NM','NY','NC','ND','OH','OK','OR','PA','RI','SC','SD','TN','TX','UT','VT','VA','WA','WV','WI','WY')")
    phone_rows = cur.fetchall()
    phone_updates = []
    phone_re = re.compile(r'\b([2-9]\d{2})\b')
    for rid, p1, p2, curr_st in phone_rows:
        st = None
        for p in (p1, p2):
            if p:
                m = phone_re.search(p)
                if m and m.group(1) in AREA_CODE_MAP:
                    ...
                    break
        if st and st != curr_st:
            phone_updates.append((st, rid))

    print(f" -> Found {len(phone_updates):,} phone area code corrections! Updating...")
    for st, rid in phone_updates:
        cur.execute("UPDATE recruiters SET state = %s WHERE recruiter_id = %s", (st, rid))
    conn.commit()
    print(" -> Point #6 committed successfully!")

    # POINT 14: Orphaned Micro-Company Reclamation
    print("\n[Point #14] Executing Orphaned Micro-Company Reclamation...")
    ...
    # POINT 18: Full PC Scanner Check for any remaining unparsed location strings
    print("\n[Point #18] Executing location string audit across database...")
    cur.execute("SELECT recruiter_id, location, state FROM recruiters WHERE location LIKE '%, __' AND RIGHT(TRIM(location), 2) != state")
    loc_rows = cur.fetchall()
    loc_updates = []
    for rid, loc, st in loc_rows:
        parsed_st = loc.strip()[-2:].upper()
        if parsed_st in VALID_STATES and parsed_st != st:
            loc_updates.append((parsed_st, rid))
    print(f" -> Found {len(loc_updates):,} location vs state discrepancies! Re-aligning...")
    for st, rid in loc_updates:
        cur.execute("UPDATE recruiters SET state = %s WHERE recruiter_id = %s", (st, rid))
    conn.commit()
    print(" -> Point #18 committed successfully!")
    ...
    cur.close()
    conn.close()
    print(f"\n=== MASTER EXECUTION COMPLETED IN {time.time() - t0:.2f}s ===")

if __name__ == "__main__":
    execute_master_engine()
