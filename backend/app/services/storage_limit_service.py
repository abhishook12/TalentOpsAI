...
# Hard limits
# DB: Strictly locked to 300 MB to prevent DB from expanding with massive storage objects
DB_SIZE_LIMIT_BYTES = 300 * 1024 * 1024

# File Storage Bucket: Hard lock at 800 MB as requested
FILE_SIZE_LIMIT_BYTES = 800 * 1024 * 1024
...
def check_database_storage_limit(db: Session):
    """Raises a 403 HTTPException if the database is over the 300MB strict lock."""
    db_size = get_database_size(db)
    if db_size >= DB_SIZE_LIMIT_BYTES:
        raise HTTPException(
            status_code=403,
            detail=f"Storage Kill-Switch Activated: Database capacity is STRICTLY LOCKED at 300MB ({db_size / 1024 / 1024:.2f} MB / 300.0 MB). ALL storage needs must be routed to File Buckets."
        )

def check_file_storage_limit(db: Session):
    """Raises a 403 HTTPException if the file storage is over the 800MB limit."""
    file_size = get_file_storage_size(db)
    if file_size >= FILE_SIZE_LIMIT_BYTES:
        raise HTTPException(
            status_code=403,
            detail=f"Storage Kill-Switch Activated: File Storage is STRICTLY LOCKED at 800MB ({file_size / 1024 / 1024:.2f} MB / 800.0 MB). Please clear space."
        )
