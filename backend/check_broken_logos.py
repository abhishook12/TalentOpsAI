...
def check_broken_logos():
    db = SessionLocal()
    companies = db.query(Company).limit(100).all() # Just test 100 for speed
    ...
    for c in companies:
        ...
        if final_domain:
            # Check Level 0: Clearbit
            try:
                res = requests.head(f"https://logo.clearbit.com/{final_domain}?size=128", timeout=3)
                if res.status_code == 200:
                    continue # It works!
            except:
                pass
            
            # Check Level 1: Google Favicon (always returns 200 with default globe if missing, hard to test via HEAD)
            
            broken_domains.add(final_domain)
            
    print(f"Sample of domains failing clearbit: {broken_domains}")
