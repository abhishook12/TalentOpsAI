#!/usr/bin/env python
"""Lightning Fast In-Memory State Triangulation Sync - TalentOpsAI"""
...
def sync_all():
    t0 = time.time()
    print(f"[{time.strftime('%X')}] FETCHING ALL UNKNOWN STATE PROFILES IN SINGLE BATCH...")
    db = SessionLocal()
    try:
        ...
        """)).mappings().all()
        print(f"[{time.strftime('%X')}] Loaded {len(rows):,} unknown state records into memory. Processing...")
        ...
        for r in rows:
            ...
            # 1. Notes
            if notes_str and '[GEO:' in notes_str:
                for s_full, s_abbr in FULL_STATE_NAMES.items():
                    if s_full in notes_str.upper(): st = s_abbr; src = "inferred_triangulation"; break
                if not st:
                    toks = re.findall(r'\b[A-Z]{2}\b', notes_str.upper())
        ...
        print(f"[{time.strftime('%X')}] In-Memory Triangulation Complete! Prepared +{len(up_batch):,} state enrichments.")

        for i in range(0, len(up_batch), 1000):
            b = up_batch[i:i+1000]
            db.execute(text("UPDATE recruiters SET state = :st, state_source = :src WHERE recruiter_id = :rid"), b)
        db.commit()

        print(f"[{time.strftime('%X')}] Successfully committed +{len(up_batch):,} state updates in {round(time.time()-t0,2)}s!")
