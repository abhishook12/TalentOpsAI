...
def main():
    ...
    manual_aliases = {
        "C&G Consulting Services, Inc": ["cgconsulting", "cgconsulting.ph"],
        "SOLTECHPR": ["soltech", "soltech.com"],
        "TJ Consulting Group": ["thetjconsultinggroup", "thetjconsultinggroup.com"],
    ...
    for idx, raw_query in enumerate(COMPANIES_LIST, 1):
        ...
        # 3. Manual Aliases
        if len(matched_pg) == 0 and raw_query in manual_aliases:
            aliases = manual_aliases[raw_query]
            mask_alias = pd.Series([False]*len(df_pg))
            for alias in aliases:
                mask_alias = mask_alias | (df_pg['norm_name'].str.contains(alias, regex=False) | df_pg['primary_domain'].fillna('').str.contains(alias, regex=False))
            matched_pg = df_pg[mask_alias]

        # 4. Local storage import
        ...
        if company_ids:
            ...
            if len(df_recs) > 0:
                sample_recs = df_recs.fillna('').to_dict(orient='records')[:2]
        ...
        if rec_count == 0 and core_tokens:
            ...
            if len(first_core) >= 5:
                ...
                if len(df_recs_dom) > 0:
                    ...
                    sample_recs = df_recs_dom.fillna('').to_dict(orient='records')[:2]

        if len(matched_local) > 0:
            rec_count += len(matched_local)
            for _, r in matched_local.head(2).iterrows():
                sample_recs.append({
        ...
        matched_comp_names = matched_pg['company_name'].unique().tolist()[:2] if len(matched_pg) > 0 else []
        matched_domains = matched_pg['primary_domain'].dropna().unique().tolist()[:2] if len(matched_pg) > 0 else []

        final_report.append({
            "index": idx,
            "company": raw_query,
            "status": "EXISTING" if is_present else "NEW_LEAD",
    ...
    with open("c:/TalentOpsAI/backend/scratch/master_111_audit_report.json", "w", encoding="utf-8") as f:
        json.dump(final_report, f, indent=2)

    found_list = [r for r in final_report if r['status'] == "EXISTING"]
    new_list = [r for r in final_report if r['status'] == "NEW_LEAD"]

    print("="*80)
    print(f"FINAL AUDIT RESULT: {len(found_list)} EXISTING (IN SYSTEM) | {len(new_list)} NEW LEADS (NOT IN SYSTEM)")
    print("="*80)
