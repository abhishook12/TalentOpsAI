...
print(f"\n=== 2. DUPLICATE MATCH RATE AT SCALE (Across all {len(group_2887) + len(group_6561_new)} candidates) ===")
...
for rec in full_pool:
    ...
    if "@" in primary_email_string:
        ...
        if matched_id:
            # Look for any recruiter at matched_id
            candidates = db.query(Recruiter).filter(Recruiter.company_id == matched_id, Recruiter.recruiter_id != rec.recruiter_id).all()
            best_dup = None
            best_score = 0
            
            for cand in candidates:
                # Compare name similarity
                sim = SequenceMatcher(None, cand.recruiter_name.lower(), misplaced_human_name.lower()).ratio()
                if sim > best_score:
                    best_score = sim
                    best_dup = cand
            
            if best_dup and best_score >= 0.8:
                # Check (a) name match >= 0.8, (b) same company_id, (c) domain match
                ...
                if best_score > 0.95 and same_co and same_dom:
                    pass_all_3 += 1
                else:
                    partial_match += 1
                    worst_matches.append((rec, best_dup, best_score, same_co, same_dom))
            else:
                no_dup_found += 1
        else:
            no_dup_found += 1
...
print(f"1. Pass ALL 3 checks automatically (Exact name >95%, Same Co ID, Same Domain): {pass_all_3}")
print(f"2. Partial matches (Fuzzy name 80-95%, or different Co ID/Domain): {partial_match}")
print(f"3. No duplicate found at all: {no_dup_found}")

print(f"\n=== 10 WORST / MOST UNCERTAIN MATCHES ===")
worst_matches.sort(key=lambda x: x[2]) # sort by lowest similarity score
for i, x in enumerate(worst_matches[:10]):
    rec, dup, score, sco, sdom = x
    print(f"\nWorst #{i+1} (NameSim: {score*100:.1f}% | SameCoID: {sco} | SameDom: {sdom}):")
