...
export default function ScoutUserProfileDrawer({ userId, onClose, onRefreshList }) {
  ...
  return (
    <div style={{
      ...
      background: 'var(--modal-backdrop-bg, rgba(0, 0, 0, 0.75))', backdropFilter: 'blur(4px)',
      display: 'flex', justifyContent: 'flex-end', animation: 'fadeIn 0.2s ease-out'
    }}>
      <div style={{
        width: '100%', maxWidth: 780, height: '100%', background: 'var(--panel-bg, #0b0b0c)',
        borderLeft: '1px solid var(--card-border, #232326)', display: 'flex', flexDirection: 'column',
        boxShadow: '-10px 0 40px rgba(0,0,0,0.5)', overflow: 'hidden'
      }}>
        {/* Drawer Header */}
        <div style={{
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6 }}>
              <h2 style={{ fontSize: 22, fontWeight: 800, color: 'var(--text-primary, #fafafa)', margin: 0, letterSpacing: '-0.3px' }}>
            ...
            <div style={{ display: 'flex', gap: 16, fontSize: 13, color: 'var(--text-secondary, #a1a1aa)' }}>
              <span>{profile?.header?.email}</span>
              <span>•</span>
              <span>Tenant: <strong style={{ color: 'var(--text-primary, #d4d4d8)' }}>{profile?.header?.tenant}</strong></span>
              <span>•</span>
              <span>Role: <strong style={{ color: 'var(--text-primary, #d4d4d8)' }}>{profile?.header?.role}</strong></span>
            </div>

            <div style={{ display: 'flex', gap: 18, marginTop: 10, fontSize: 11, color: 'var(--text-muted, #71717a)' }}>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            <button
              onClick={() => refetch()}
              style={{ padding: '6px 10px', background: 'var(--card-bg, #232326)', color: 'var(--text-secondary, #a1a1aa)', border: '1px solid var(--card-border, #27272a)', borderRadius: 6, cursor: 'pointer' }}
            ...
            <button
              onClick={onClose}
              style={{ padding: '6px 10px', background: 'var(--card-bg, #232326)', color: 'var(--text-primary, #fafafa)', border: '1px solid var(--card-border, #27272a)', borderRadius: 6, cursor: 'pointer' }}
              title="Close Drawer"
            >
              <X size={16} />
            </button>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px' }}>
          {isLoading ? (
            <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted, #a1a1aa)' }}>
            ...
            <>
              ...
              {activeTab === 'contributions' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                  ...
                  <div style={{
                    padding: '16px 20px', borderRadius: 12, background: 'var(--bg-base, rgba(212, 212, 216,0.08))',
                    border: '1px solid var(--card-border, rgba(212, 212, 216,0.2))', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20
                  }}>
                    <div>
                      <div style={{ fontSize: 11, color: 'var(--text-secondary, #f4f4f5)', fontWeight: 700, textTransform: 'uppercase' }}>
                        Raw Ingestion Volume
                      </div>
                      <div style={{ fontSize: 26, fontWeight: 900, color: 'var(--text-primary, #fafafa)', margin: '4px 0' }}>
                  ...
                  <div style={{ background: 'var(--card-bg, #0e1526)', border: '1px solid var(--card-border, #232326)', borderRadius: 12, padding: '18px 20px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}>
                      <div>
                        <div style={{ fontSize: 11, color: 'var(--text-muted, #71717a)' }}>Direct Emails</div>
                        <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--text-primary, #fafafa)', marginTop: 2 }}>
                          {profile?.contributions?.emails_discovered || 0}
                        </div>
                      </div>
                      <div>
                        <div style={{ fontSize: 11, color: 'var(--text-muted, #71717a)' }}>Phone Numbers</div>
                        <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--text-primary, #fafafa)', marginTop: 2 }}>
                          {profile?.contributions?.phones_discovered || 0}
                        </div>
                      </div>
                      <div>
                        <div style={{ fontSize: 11, color: 'var(--text-muted, #71717a)' }}>LinkedIn Profiles</div>
                        <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--text-primary, #fafafa)', marginTop: 2 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                  ...
                  <div style={{ background: 'var(--card-bg, #0e1526)', border: '1px solid var(--card-border, #232326)', borderRadius: 12, padding: '20px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
                      <div>
                        <div style={{ fontSize: 11, color: 'var(--text-muted, #71717a)' }}>Canonical Entities Created</div>
                        <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--text-primary, #e4e4e7)', marginTop: 2 }}>
                      ...
                      <div>
                        <div style={{ fontSize: 11, color: 'var(--text-muted, #71717a)' }}>Duplicates Prevented</div>
                        <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--text-secondary, #a1a1aa)', marginTop: 2 }}>
                      ...
                      <div>
                        <div style={{ fontSize: 11, color: 'var(--text-muted, #71717a)' }}>Low Confidence Rejections</div>
              ...
              {activeTab === 'devices' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  {(profile?.devices || []).length === 0 ? (
                    <div style={{ padding: 30, textAlign: 'center', color: 'var(--text-muted, #71717a)' }}>
                      ...
                      <div key={idx} style={{
                        ...
                        <div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
                            ...
                            <span style={{ fontSize: 11, color: 'var(--text-muted, #71717a)' }}>v{dev.scout_version}</span>
                          </div>

                          <div style={{ fontSize: 11, color: 'var(--text-muted, #71717a)', display: 'flex', gap: 14 }}>
                            <span>Device ID: <strong style={{ color: 'var(--text-secondary, #a1a1aa)' }}>{dev.device_id}</strong></span>
                            <span>OS: <strong style={{ color: 'var(--text-secondary, #a1a1aa)' }}>{dev.os}</strong></span>
                            <span>Queue: <strong style={{ color: 'var(--text-secondary, #a1a1aa)' }}>{dev.queue_size} items</strong></span>
                            <span>Last Seen: <strong style={{ color: 'var(--text-secondary, #a1a1aa)' }}>{dev.last_seen ? dev.last_seen.slice(0, 16).replace('T', ' ') : '—'}</strong></span>
                          </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                          <button
                            ...
                            style={{
                              padding: '6px 12px', background: 'var(--panel-bg, #232326)', color: 'var(--text-primary, #e4e4e7)',
                              border: '1px solid var(--card-border, #27272a)', borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: 'pointer'
                            }}
                          >
                            Force Update
                          </button>
              ...
              {activeTab === 'sources' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  <div style={{ fontSize: 13, color: 'var(--text-secondary, #a1a1aa)', marginBottom: 6 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  ...
                  {(profile?.provenance_trail || []).map((p, idx) => (
                    <div key={idx} style={{
                      ...
                      <div>
                        <div style={{ fontWeight: 700, color: 'var(--text-primary, #fafafa)', display: 'flex', alignItems: 'center', gap: 8 }}>
                          <span>{p.candidate_name}</span>
                          <span style={{ color: 'var(--text-secondary, #a1a1aa)' }}>• {p.company_name} ({p.title})</span>
              ...
              {activeTab === 'actions' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  <div style={{ padding: '16px 20px', background: 'var(--card-bg, #0e1526)', border: '1px solid var(--card-border, #232326)', borderRadius: 12 }}>
                    ...
                    <p style={{ fontSize: 12, color: 'var(--text-muted, #71717a)', margin: '0 0 14px' }}>
