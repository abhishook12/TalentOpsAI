...
export default function ScoutUserProfileDrawer({ userId, onClose, onRefreshList }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        width: '100%', maxWidth: 780, height: '100%', background: '#090d16',
        ...
        <div style={{
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6 }}>
              <h2 style={{ fontSize: 22, fontWeight: 800, color: '#f8fafc', margin: 0, letterSpacing: '-0.3px' }}>
            ...
            <div style={{ display: 'flex', gap: 16, fontSize: 13, color: '#94a3b8' }}>
              ...
              <span>•</span>
              <span>Tenant: <strong style={{ color: '#cbd5e1' }}>{profile?.header?.tenant}</strong></span>
              <span>•</span>
              <span>Role: <strong style={{ color: '#cbd5e1' }}>{profile?.header?.role}</strong></span>
            </div>

            <div style={{ display: 'flex', gap: 18, marginTop: 10, fontSize: 11, color: '#64748b' }}>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            <button
              onClick={() => refetch()}
              style={{ padding: '6px 10px', background: '#1e293b', color: '#94a3b8', border: '1px solid #334155', borderRadius: 6, cursor: 'pointer' }}
            ...
            <button
              onClick={onClose}
              style={{ padding: '6px 10px', background: '#1e293b', color: '#f8fafc', border: '1px solid #334155', borderRadius: 6, cursor: 'pointer' }}
              title="Close Drawer"
            >
              <X size={16} />
            </button>
        ...
        {/* Tab Navigation */}
        <div style={{ display: 'flex', gap: 6, padding: '12px 28px', borderBottom: '1px solid #1e293b', background: '#0c121e' }}>
          {[
            ...
            <>
              ...
              {activeTab === 'contributions' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                  ...
                  <div style={{
                    padding: '16px 20px', borderRadius: 12, background: 'rgba(59,130,246,0.08)',
                    border: '1px solid rgba(59,130,246,0.2)', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20
                  }}>
                    <div>
                      <div style={{ fontSize: 11, color: '#93c5fd', fontWeight: 700, textTransform: 'uppercase' }}>
                        Raw Ingestion Volume
                      </div>
                      <div style={{ fontSize: 26, fontWeight: 900, color: '#f8fafc', margin: '4px 0' }}>
                    ...
                    <div style={{ borderLeft: '1px solid rgba(59,130,246,0.2)', paddingLeft: 20 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
                    {[
                      { label: 'New People', val: profile?.contributions?.people_created, color: '#38bdf8' },
                      ...
                      { label: 'Contacts (Emails/Phones)', val: profile?.contributions?.contacts_discovered, color: '#a78bfa' },
                    ].map((card, i) => (
                      <div key={i} style={{ padding: '14px 16px', background: '#0e1526', border: '1px solid #1e293b', borderRadius: 10 }}>
                  ...
                  <div style={{ background: '#0e1526', border: '1px solid #1e293b', borderRadius: 12, padding: '18px 20px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      {(profile?.timeline || []).slice(-7).map((t, idx) => (
                        <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid #1e293b', fontSize: 12 }}>
                          <span style={{ color: '#94a3b8' }}>{t.date}</span>
                          <span style={{ color: '#4ade80', fontWeight: 700 }}>+{t.people} people</span>
                          <span style={{ color: '#38bdf8' }}>+{t.companies} companies</span>
                          <span style={{ color: '#a78bfa' }}>+{t.contacts} contacts</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                    {[
                      ...
                      <div key={i} style={{
                        ...
                        border: `1px solid ${q.highlight ? 'rgba(16,185,129,0.35)' : '#1e293b'}`
                      }}>
                        <div style={{ fontSize: 11, color: q.highlight ? '#4ade80' : '#64748b', fontWeight: 700 }}>{q.label}</div>
                        <div style={{ fontSize: 24, fontWeight: 900, color: q.highlight ? '#10b981' : '#f8fafc', margin: '4px 0' }}>
                          {q.val || 0}%
                        </div>
                        <div style={{ fontSize: 10, color: '#64748b', lineHeight: 1.4 }}>{q.desc}</div>
                      </div>
                    ))}
                  </div>

                  {/* Downstream Impact Details */}
                  <div style={{ background: '#0e1526', border: '1px solid #1e293b', borderRadius: 12, padding: '20px' }}>
                    <div style={{ fontSize: 14, fontWeight: 700, color: '#cbd5e1', marginBottom: 16 }}>
                      Canonical Master Database Impact
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
                      <div>
                        <div style={{ fontSize: 11, color: '#64748b' }}>Canonical Entities Created</div>
                        <div style={{ fontSize: 18, fontWeight: 800, color: '#38bdf8', marginTop: 2 }}>
                          {profile?.data_quality_impact?.canonical_entities_created || 0}
                        </div>
                      </div>
                      <div>
                        <div style={{ fontSize: 11, color: '#64748b' }}>Existing Entities Enriched</div>
                        <div style={{ fontSize: 18, fontWeight: 800, color: '#4ade80', marginTop: 2 }}>
                          {profile?.data_quality_impact?.existing_entities_enriched || 0}
                        </div>
                      </div>
                      <div>
                        <div style={{ fontSize: 11, color: '#64748b' }}>Corrections Proposed / Accepted</div>
                        <div style={{ fontSize: 18, fontWeight: 800, color: '#fbbf24', marginTop: 2 }}>
                          {profile?.data_quality_impact?.corrections_accepted || 0}
                        </div>
                      </div>
                      <div>
                        <div style={{ fontSize: 11, color: '#64748b' }}>Duplicates Prevented</div>
                        <div style={{ fontSize: 18, fontWeight: 800, color: '#94a3b8', marginTop: 2 }}>
                          {profile?.data_quality_impact?.duplicates_detected || 0}
                        </div>
                      </div>
                      <div>
                        <div style={{ fontSize: 11, color: '#64748b' }}>Quarantined for Review</div>
                        <div style={{ fontSize: 18, fontWeight: 800, color: '#f59e0b', marginTop: 2 }}>
                          {profile?.data_quality_impact?.quarantined_observations || 0}
                        </div>
                      </div>
                      <div>
                        <div style={{ fontSize: 11, color: '#64748b' }}>Low Confidence Rejections</div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  {(profile?.devices || []).length === 0 ? (
                    ...
                    (profile?.devices || []).map((dev, idx) => (
                      <div key={idx} style={{
                        background: '#0e1526', border: '1px solid #1e293b', borderRadius: 12, padding: '18px 20px',
                        ...
                        <div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
                            ...
                            <span style={{ fontSize: 11, color: '#64748b' }}>v{dev.scout_version}</span>
                          </div>

                          <div style={{ fontSize: 11, color: '#64748b', display: 'flex', gap: 14 }}>
                            <span>Device ID: <strong style={{ color: '#94a3b8' }}>{dev.device_id}</strong></span>
                            <span>OS: <strong style={{ color: '#94a3b8' }}>{dev.os}</strong></span>
                            <span>Queue: <strong style={{ color: '#94a3b8' }}>{dev.queue_size} items</strong></span>
                            <span>Last Seen: <strong style={{ color: '#94a3b8' }}>{dev.last_seen ? dev.last_seen.slice(0, 16).replace('T', ' ') : '—'}</strong></span>
                          </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                          <button
                            ...
                            style={{
                              padding: '6px 12px', background: '#1e293b', color: '#38bdf8',
                              border: '1px solid #334155', borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: 'pointer'
                            }}
                          >
                            Force Update
                          </button>
              ...
              {activeTab === 'provenance' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  ...
                  {(profile?.provenance_trail || []).map((p, idx) => (
                    <div key={idx} style={{
                      padding: '12px 16px', background: '#0e1526', border: '1px solid #1e293b', borderRadius: 8,
                      display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 12
                    }}>
                      <div>
                        <div style={{ fontWeight: 700, color: '#f8fafc' }}>
                          {p.candidate_name} • <span style={{ color: '#94a3b8' }}>{p.company_name} ({p.title})</span>
                        </div>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        <span style={{
                          ...
                          background: p.decision === 'ENRICHED' ? 'rgba(16,185,129,0.15)' : 'rgba(56,189,248,0.15)',
                          color: p.decision === 'ENRICHED' ? '#4ade80' : '#38bdf8'
                        }}>
                          {p.decision}
                        </span>
              ...
              {activeTab === 'actions' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  <div style={{ padding: '16px 20px', background: '#0e1526', border: '1px solid #1e293b', borderRadius: 12 }}>
