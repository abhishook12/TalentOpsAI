    ...
    <div style={{ flexShrink: 0, background: 'var(--panel-bg)', border: `1px solid ${isOpen ? 'var(--card-border)' : 'var(--card-border)'}`, borderRadius: 10, overflow: 'hidden' }}>
      ...
      >
        <div style={{ width: 28, height: 28, borderRadius: 7, background: 'var(--card-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <i className={`ti ${browserIcon}`} style={{ fontSize: 14, color: '#e4e4e7' }} />
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: session.user_email === 'Anonymous' ? '#a1a1aa' : '#e4e4e7', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {session.user_email}
          </div>
        ...
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: '#e4e4e7', fontFamily: "'DM Mono', monospace" }}>{duration}</div>
          <div style={{ fontSize: 10, color: '#71717a' }}>{session.page_count} pg</div>
        </div>
        ...
        <div style={{ borderTop: '1px solid #111c30', padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 5 }}>
          ...
          {session.pages.map((p, pi) => (
            <div key={pi} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: '#a1a1aa' }}>
              <span style={{ color: '#3f3f46', minWidth: 16 }}>{pi + 1}.</span>
              <span style={{ flex: 1 }}>{p}</span>
              <span style={{ color: '#71717a', fontFamily: "'DM Mono', monospace" }}>{String(session.timestamps[pi] || '').slice(11, 19)}</span>
            </div>
          ))}
        </div>
      )}
