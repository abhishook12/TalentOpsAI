...
export default function ActivityLog() {
  ...
  const fetchDatabaseActivity = async () => {
    try {
      ...
      console.error("Failed to fetch DB activity", error);
    } finally {
      setDbLoading(false);
    }
  };
  ...
  const getDbBadgeLabel = (category) => {
    switch (category) {
      case "added": return "ADDED";
      case "improved": return "IMPROVED";
      case "needs_improvement": return "UPDATED";
      case "removed": return "REMOVED";
      default: return "UPDATED";
    }
  };

  const getDbBadgeClasses = (category) =>
  ...
  return (
    <div className="min-h-screen bg-background text-on-surface selection:bg-primary selection:text-on-primary">
      <style>{`
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb { background: #333535; border-radius: 2px; }
        .activity-pulse { position: relative; }
        .activity-pulse::after { content: ''; position: absolute; left: 0; top: 0; bottom: 0; width: 2px; background: #ffffff; box-shadow: 0 0 8px rgba(255,255,255,0.3); }
      ...
      <main className="mx-auto min-h-screen max-w-2xl px-4 pb-24 pt-24">
        <div className="mb-8">
          <p className="mb-1 text-xs font-mono uppercase tracking-[0.2em] text-on-surface-variant">
            {activeTab === 'database' ? 'Global Activity Log' : 'Live Traffic Monitor'}
          </p>
          <div className="flex items-baseline justify-between">
            <h2 className="text-3xl font-bold text-on-surface">
              {activeTab === 'database' ? 'Database Activity' : 'Visitor Logs'}
            </h2>
            <div className="flex items-center gap-3">
              <button onClick={fetchAll} className="flex h-8 w-8 items-center justify-center rounded-full border border-outline-variant bg-surface-container-low transition-colors hover:bg-surface-container-high active:scale-95">
          ...
          <p className="mt-1 text-sm text-on-surface-variant">
            {activeTab === 'database' ? 'Real-time database mutations and enrichments.' : 'Real-time human traffic and session tracking.'}
          </p>
        </div>
          ...
          <>
            ...
            <div className="mb-6 flex gap-2 overflow-x-auto pb-4 no-scrollbar">
              {dbCategoryOptions.map((cat) => {
                const isActive = dbFilter === cat.key;
                return (
                  <button
                    key={cat.key}
            ...
            <div className="space-y-3">
              <AnimatePresence>
                {filteredActivity.map((item, index) => {
                  const label = getDbBadgeLabel(item.category);
                  const badgeClasses = getDbBadgeClasses(item.category);
                ...
                {filteredActivity.length === 0 && (
                  <div className="rounded-lg border border-dashed border-outline-variant p-8 text-center font-mono text-sm text-on-surface-variant">
                    {dbLoading ? "Loading live data streams..." : "No activity found for this filter."}
                  </div>
                )}
              </AnimatePresence>
            </div>
            ...
            <AnimatePresence>
              {visitors.map((v, index) => (
                <motion.div
                  ...
                  animate={{ opacity: 1, scale: 1 }}
                  exit={{ opacity: 0, scale: 0.95 }}
                  className="flex flex-col gap-2 rounded-lg border border-outline-variant bg-surface-container p-4 transition-all hover:bg-surface-container-high active:scale-[0.98]"
                >
                  <div className="flex items-start justify-between">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-on-surface flex items-center gap-2">
                      {v.page}
                      <span className="text-[10px] font-mono font-normal text-on-surface-variant bg-surface-container-high px-2 py-0.5 rounded">
                        {v.path}
                      </span>
                    </h3>
                  </div>
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-y-2 gap-x-4 text-[11px] font-mono">
                    <div className="flex items-center gap-1.5 overflow-hidden">
                      <i className="ti ti-device-desktop text-[14px] text-primary" />
                      <span className="truncate text-on-surface">{v.os} • {v.browser}</span>
                    </div>
                    <div className="flex items-center gap-1.5 overflow-hidden">
                      <i className="ti ti-network text-[14px] text-on-surface-variant" />
                      <span className="truncate uppercase text-on-surface">{v.ip_address}</span>
                    </div>
                    <div className="flex items-center gap-1.5 overflow-hidden col-span-2 opacity-60" title={v.raw_ua}>
                      <i className="ti ti-fingerprint text-[14px] text-on-surface-variant" />
                      <span className="truncate text-on-surface-variant">{v.raw_ua || 'Unknown Device'}</span>
                    </div>
                  </div>
                </motion.div>
              ))}
              ...
              {visitors.length === 0 && (
                <div className="rounded-lg border border-dashed border-outline-variant p-8 text-center font-mono text-sm text-on-surface-variant">
                  {visLoading ? "Scanning for active visitors..." : "No visitor data found."}
                </div>
              )}
            </AnimatePresence>
          </div>
