...
function Modal({ title, onClose, onSave, form, setForm, saving, candidates, recruiters, companies, vendors }) {
  return (
    <div style={{
      ...
      <div style={{
        background: 'var(--card-bg)', borderRadius: 14, width: 540, maxHeight: '90vh',
        ...
        <div style={{ padding: '20px 24px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          ...
          <div>
            <label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: 'var(--text-secondary)', marginBottom: 5 }}>Recruiter</label>
          ...
          <div>
            <label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: 'var(--text-secondary)', marginBottom: 5 }}>Company</label>
          ...
          <div style={{ gridColumn: 'span 2' }}>
            <label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: 'var(--text-secondary)', marginBottom: 5 }}>Job Title</label>
          ...
          <div>
            <label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: 'var(--text-secondary)', marginBottom: 5 }}>Submission Date</label>
          ...
          <div style={{ gridColumn: 'span 2' }}>
            ...
            <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} rows={3}
              style={{ width: '100%', padding: '9px 12px', border: '1px solid var(--card-border)', borderRadius: 8, fontSize: 13.5, outline: 'none', resize: 'vertical', fontFamily: 'inherit' }} />
          </div>
        </div>
  ...
  return (
    <div className="page-enter">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 500, color: 'var(--text-primary)', letterSpacing: '-0.02em', marginBottom: 3 }}>Submissions</h1>
          <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>{filtered.length} submission{filtered.length !== 1 ? 's' : ''} tracked</p>
        </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 18, flexWrap: 'wrap' }}>
        <button onClick={() => setFilterStatus('')}
          style={{ padding: '5px 14px', borderRadius: 999, border: `1.5px solid ${filterStatus === '' ? 'var(--accent)' : 'var(--card-border)'}`, background: filterStatus === '' ? 'var(--accent)' : 'var(--card-bg)', color: filterStatus === '' ? '#fff' : 'var(--text-secondary)', fontSize: 12, cursor: 'pointer', fontWeight: 500 }}>
          All ({submissions.length})
        </button>
        {STATUSES.map(s => {
          const count = submissions.filter(x => x.status === s).length
          const st = statusStyle[s] || { bg: 'var(--bg-hover)', color: 'var(--text-secondary)' }
          return (
            <button key={s} onClick={() => setFilterStatus(s === filterStatus ? '' : s)}
              style={{ padding: '5px 14px', borderRadius: 999, border: `1.5px solid ${filterStatus === s ? st.color : 'var(--card-border)'}`, background: filterStatus === s ? st.bg : 'var(--card-bg)', color: filterStatus === s ? st.color : 'var(--text-secondary)', fontSize: 12, cursor: 'pointer', fontWeight: 500 }}>
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        {loading ? (
          <div style={{ padding: 48, textAlign: 'center', color: 'var(--text-muted)', fontSize: 13 }}>Loading submissions...</div>
        ) : filtered.length === 0 ? (
          ...
          <table>
            ...
            <tbody>
              {filtered.map(s => {
                const st = statusStyle[s.status] || { bg: 'var(--bg-hover)', color: 'var(--text-secondary)' }
                const next = NEXT_STATUS[s.status]
                return (
                  <tr key={s.submission_id}>
                    <td>
                      <div style={{ fontWeight: 500, color: 'var(--text-primary)', fontSize: 13.5 }}>{s.candidate_name || `#${s.candidate_id}`}</div>
                    </td>
                    <td style={{ color: 'var(--text-secondary)', fontSize: 13 }}>{s.job_title || '—'}</td>
                    <td style={{ color: 'var(--text-secondary)', fontSize: 13 }}>{s.company_name || '—'}</td>
                    <td style={{ color: 'var(--text-secondary)', fontSize: 13 }}>{s.recruiter_name || '—'}</td>
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        ...
                        {next && (
                          <button onClick={() => updateStatus(s.submission_id, next)}
                            title={`Move to ${next}`}
                            style={{ padding: '3px 8px', borderRadius: 6, border: '1px solid var(--card-border)', background: 'var(--card-bg)', color: 'var(--text-secondary)', fontSize: 11, cursor: 'pointer' }}>
                            → {next}
                          </button>
                        )}
                      </div>
                    </td>
                    <td style={{ color: 'var(--text-muted)', fontSize: 12 }}>{s.submission_date || '—'}</td>
                    <td>
                      <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                        <button onClick={() => openEdit(s)} style={{ padding: '5px 10px', borderRadius: 6, border: '1px solid var(--card-border)', background: 'var(--card-bg)', color: 'var(--text-secondary)', fontSize: 12, cursor: 'pointer' }}>
                          <i className="ti ti-edit" style={{ fontSize: 13 }} />
                        </button>
                        <button onClick={() => handleDelete(s.submission_id)} style={{ padding: '5px 10px', borderRadius: 6, border: '1px solid rgba(239,68,68,0.2)', background: 'var(--card-bg)', color: '#ef4444', fontSize: 12, cursor: 'pointer' }}>
