...
from .components import (
    Card, PageHead, StateChip, ConfidenceMeter,
...
class ActivityPage(QWidget):
    ...
    def _build_ui(self):
        ...
        # Mark all read action button
        btn_mark = QPushButton("Mark all read")
        btn_mark.setFont(QFont("Segoe UI", 8, QFont.Weight.DemiBold))
        ...
        """)
        btn_mark.clicked.connect(self._on_mark_all_read)

        head = PageHead(
            "Activity",
            "Audit log of edge intelligence decisions and sync telemetry.",
            action_widget=btn_mark
        )
        main_layout.addWidget(head)
    ...
    def _render_feed(self):
        ...
        for act in ACTIVITY_FEED:
            ...
            row_card = self._create_activity_card(act)
            self.feed_layout.addWidget(row_card)
    ...
    def _create_activity_card(self, act: Dict[str, Any]) -> QWidget:
        ...
        # Unread dot
        dot = QLabel("•" if act.get("unread") else "")
        ...
        lbl_title.setFont(QFont("Segoe UI", 9, QFont.Weight.Medium))
        lbl_title.setStyleSheet(f"color: {COLOR_TEXT_PRIMARY};")
        col.addWidget(lbl_title)

        lbl_dt = QLabel(act.get("detail", ""))
        lbl_dt.setFont(QFont("Segoe UI", 7))
        lbl_dt.setStyleSheet(f"color: {COLOR_TEXT_MUTED};")
...
class SettingsPage(QWidget):
    """
    Settings Screen:
    - Authorized sources list with toggles
    - Gate confidence thresholds
    - Device identity (Installation #483, Device ID, Tenant ID)
    - Extractor & App versions
    - Update settings
    """
    def __init__(self, parent: Optional[QWidget] = None):
        super().__init__(parent)
    ...
    def _build_ui(self):
        ...
        head = PageHead(
            "Settings",
            "Local edge configuration, detection policies, and device claims."
        )
        main_layout.addWidget(head)
        ...
        # 1. Authorized Sources Card
        card_sources = Card()
        src_layout = QVBoxLayout(card_sources)
        src_layout.setContentsMargins(16, 14, 16, 14)
        src_layout.setSpacing(8)

        lbl_src_title = QLabel("Authorized Sources & Windows")
        lbl_src_title.setFont(QFont("Segoe UI", 10, QFont.Weight.Bold))
        lbl_src_title.setStyleSheet(f"color: {COLOR_TEXT_PRIMARY};")
        src_layout.addWidget(lbl_src_title)

        for src in SETTINGS_DATA["authorized_sources"]:
            row = QHBoxLayout()
            chk = QCheckBox(src["name"])
            chk.setChecked(src["enabled"])
            chk.setFont(QFont("Segoe UI", 8))
            chk.setStyleSheet(f"color: {COLOR_TEXT_SECONDARY};")
            row.addWidget(chk)
            row.addStretch()

            lbl_cat = QLabel(src["category"])
            lbl_cat.setFont(QFont("Segoe UI", 7))
            lbl_cat.setStyleSheet(f"color: {COLOR_TEXT_MUTED};")
            row.addWidget(lbl_cat)

            src_layout.addLayout(row)

        c_layout.addWidget(card_sources)

        # 2. Gate Confidence Thresholds Card
        card_thresh = Card()
        th_layout = QVBoxLayout(card_thresh)
        th_layout.setContentsMargins(16, 14, 16, 14)
        th_layout.setSpacing(8)

        lbl_th_title = QLabel("Gate Confidence Thresholds")
        ...
        th = SETTINGS_DATA["gate_thresholds"]
        th_layout.addWidget(ConfidenceMeter(f"Auto-Commit Threshold ({th['auto_commit']})", int(th['auto_commit'] * 100)))
        th_layout.addWidget(ConfidenceMeter(f"Review Threshold ({th['review_threshold']})", int(th['review_threshold'] * 100)))

        c_layout.addWidget(card_thresh)

        # 3. Device Identity Card
        card_dev = Card()
        ...
        dev = SETTINGS_DATA["device_identity"]
        for k, v in [("Installation ID", dev["installation_id"]),
                     ("Device ID", dev["device_id"]),
                     ("Tenant ID", dev["tenant_id"]),
                     ("Registered To", dev["registered_to"])]:
            r = QHBoxLayout()
            lbl_k = QLabel(k)
        ...
        c_layout.addWidget(card_dev)

        # 4. Engine & OS Versions Card
        card_ver = Card()
        ver_layout = QVBoxLayout(card_ver)
        ver_layout.setContentsMargins(16, 14, 16, 14)
        ver_layout.setSpacing(6)

        lbl_v_title = QLabel("Engine & Environment")
        lbl_v_title.setFont(QFont("Segoe UI", 10, QFont.Weight.Bold))
        lbl_v_title.setStyleSheet(f"color: {COLOR_TEXT_PRIMARY};")
        ver_layout.addWidget(lbl_v_title)

        v_data = SETTINGS_DATA["versions"]
        for k, v in [("Scout Version", v_data["scout_version"]),
                     ("Extractor Version", v_data["extractor_version"]),
                     ("OS Version", v_data["os_version"]),
                     ("Deployment Channel", v_data["channel"])]:
            r = QHBoxLayout()
            lbl_k = QLabel(k)
            ...
            r.addWidget(lbl_v)
            r.addStretch()
            ver_layout.addLayout(r)

        c_layout.addWidget(card_ver)
...
class SignInClaimPage(QWidget):
    """
    Sign-in & Device Claim Screen:
    - Short-lived claim code entry ('TOS-XXXX-XXXX')
    - 'Sign in with TalentOps account' alternative
    - Trust notes (No passwords stored, Identity survives updates, Only signed releases install)
    ...
    def _build_ui(self):
        ...
        lbl_icon = QLabel("👁️")
        lbl_icon.setFont(QFont("Segoe UI", 16))
        head_row.addWidget(lbl_icon)
        ...
        self.lbl_card_sub = QLabel("Connect this installation to your TalentOps account")
        self.lbl_card_sub.setFont(QFont("Segoe UI", 9))
        self.lbl_card_sub.setStyleSheet(f"color: {COLOR_TEXT_SECONDARY};")
        card_layout.addWidget(self.lbl_card_sub)

        # Claim Code input box
        self.txt_claim_code = QLineEdit()
        self.txt_claim_code.setPlaceholderText("TOS-XXXX-XXXX")
        self.txt_claim_code.setText("TOS-4831-9204")
        self.txt_claim_code.setFixedHeight(40)
        self.txt_claim_code.setFont(QFont("Consolas", 11, QFont.Weight.Bold))
        ...
        # Action Buttons
        self.btn_claim = QPushButton("Claim Device & Start Observing")
        ...
        self.btn_account_signin.setFixedHeight(34)
        ...
        # Success Message Widget (Hidden by default)
        ...
        self.btn_start_observing.setFixedHeight(36)
        ...
        div.setStyleSheet(f"color: {COLOR_SURFACE_BORDER};")
        card_layout.addWidget(div)

        # Trust Notes
        lbl_trust_title = QLabel("Security & Trust")
        lbl_trust_title.setFont(QFont("Segoe UI", 7, QFont.Weight.Bold))
        lbl_trust_title.setStyleSheet(f"color: {COLOR_TEXT_MUTED};")
        card_layout.addWidget(lbl_trust_title)

        for note in DEVICE_CLAIM_STATE["trust_notes"]:
            lbl_n = QLabel(note)
            lbl_n.setFont(QFont("Segoe UI", 7))
            lbl_n.setStyleSheet(f"color: {COLOR_TEXT_MUTED};")
    ...
    def _on_claim_click(self):
        ...
        self.lbl_card_sub.setText("Device linked to your enterprise account.")
